...
function [ngrad_b, ngrad_W]= Assignment1_bonus1()
% To find the bonus parts of the assignment search for: BONUS
...
% 
%%% Plotting loss and costs computed after each epoch during the gradient
%%% descent
...
%
%%%% Plot image representation of weights 
% plotWeights(Wstar);
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%%% BONUS a, Using all avialable training data %%%%%
% Performing gradient descent with all available training data except for
% 1000 data points used for validation
...
%Concatenating data and removing 1000 datapoints that are going to be used for validation
Xtr = [X1, X2, X3, X4, X5(:,1:9000)];
Ytr = [Y1, Y2, Y3, Y4, Y5(:,1:9000)];
ytr = [y1; y2; y3; y4; y5(1:9000)];

%Extracting the last data points for validation
Xva = X5(:,9001:10000);
Yva = Y5(:,9001:10000);
yva = y5(9001:10000);
...
%plotting the loss and costs values computed after each epoch 
plotResults(cost_training, cost_validation, GDparameters.n_epochs , 'training cost', 'validation cost', '(a), (d) & (g) - Mini batch graddient descent costs', 'epoch', 'cost');
plotResults(loss_training, loss_validation, GDparameters.n_epochs , 'training loss', 'validation loss', '(a), (d) & (g) - Mini batch graddient descent losses', 'epoch', 'loss');
...
% plots either cost or loss validations performed on the training and
...
% create vectors used to store results if the computed costs and losses
...
for i = 1:GDparams.n_epochs
    ...
    %%% BONUS (g) %%%%
    [ Xtr, Ytr ] = shuffleDataPoints(Xtr, Ytr);
    %%%%%%%%%%%%%%%%%%
...
%creates q matrix of one hot encoded data from an array of labels  
...
% computes the cost and the loss 
...
% Computes the final accuracy using the final weights and biases. These
% results are tahn compared to the ground truth provided by y
...
% Computes the gradient of W and b via efficient matrix computations
% described during the lectures.
...
function[ X_shuffled, Y_shuffled ] = shuffleDataPoints(X, Y)
